...
use crate::collectors::{procscan, runners};
use crate::hooks::install;
use crate::hooks::uninstall::{self as hook_revert, RevertAction, RunnerHookPlan};
use crate::paths::{self, Scope};
use crate::{privileged, systemd};
...
fn discover_runners(config_override: Option<&Path>) -> Vec<crate::model::RunnerInfo> {
    let roots = crate::config::Config::load(config_override)
        .ok()
