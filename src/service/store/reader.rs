...
pub fn latest_api_runners(
    ...
    let rows = stmt.query_map([], |r| {
        ...
        // Compare in u64 space. Casting `max_age` down to i64 would wrap a large
        // window (u64::MAX ⇒ -1) and mark every row stale — the exact inverse of
        // the intent. `age_s` is clamped non-negative, so widening it is lossless.
        let view = if age_s as u64 <= max_age {
            GhView::Fresh {
                state: ApiState {
