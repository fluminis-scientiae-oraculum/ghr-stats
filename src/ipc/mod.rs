...
//! Every response payload reuses a `store::reader` struct verbatim, so the wire
//! types and the DB query types can never drift apart.
...
/// A TUI → collector query. One variant per `store::reader` fn the TUI needs,
/// plus a `Hello` handshake that proves the peer speaks our protocol version.
